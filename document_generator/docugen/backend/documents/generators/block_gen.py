...
def render_docx(ctx: GenerationContext) -> bytes:
    ...
    if cfg["include_suivi"]:
        ...
        for i, h in enumerate(["Version", "Date", "Par", "Commentaire"]):
            ...
            if rt.rows[0].cells[i].paragraphs[0].runs:
        ...
        if bt == "heading":
            ...
            if columns:
                ...
                for i, col in enumerate(columns):
                    ...
                    if tt.rows[0].cells[i].paragraphs[0].runs:
...
def render_pdf(ctx: GenerationContext) -> bytes:
    ...
    if cfg["include_suivi"]:
        ...
        t.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, 0), _hex(HS.COLORS["primary"])),
    ...
    for block in ctx.document.template.schema:
        ...
        if bt == "heading":
            ...
            story += _contacts_pdf(block, ctx, heading_styles[3], body_font, ms_sb)
        elif bt == "spacer":
...
def _contacts_pdf(block, ctx, heading_style, body_font, bold_font):
