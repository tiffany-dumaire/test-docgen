...
- **Modèles PowerPoint (.pptx)** : nouveau format de document. Génère une
  diapositive de titre + des diapositives de contenu à partir du schéma par
  blocs (titres, textes, listes, tableaux, images), avec styles configurables
  (couleur de thème, polices titre/corps) inspirés d'un modèle PowerPoint.
