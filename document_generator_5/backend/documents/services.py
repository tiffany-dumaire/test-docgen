...
def generate_version(document: Document, *, author_initials, author_name="",
    ...
    if document.doc_type == "xlsx" and excel_cfg.get("sheets"):
        ...
        if document.doc_type == "pdf":
            # PDF IDENTIQUE AU WORD : on génère le .docx (charte du modèle) puis
            # on le convertit en PDF via MICROSOFT WORD (docx2pdf). Repli reportlab
            # uniquement si Word est indisponible (ex. environnement de dev Linux).
