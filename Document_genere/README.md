...
- **Documents** générés à partir de **modèles paramétrables** :
  - PDF de suivi de projet (jalons, contacts, historique) ;
  - Excel générique (feuille d'informations, données, historique) ;
  - Word d'analyse (résumé exécutif, sections, recommandations).
  - Chaque modèle définit un **schéma de variables** (JSON) alimenté depuis l'interface.
...
2. **Créez un projet** : menu « Projets » → « Nouveau projet ». Ajoutez les contacts
   client et internes.
3. **Créez un document** : menu « Documents » → « Nouveau document ». Choisissez le
   projet et un modèle (PDF, Excel ou Word), définissez le niveau de confidentialité,
   remplissez les variables, puis **enregistrez**.
4. **Générez** : dans l'éditeur du document, renseignez vos initiales et un commentaire,
   puis cliquez sur **Générer**. Le fichier est produit et ajouté à l'historique.
   Toute modification suivie d'une **régénération** crée une nouvelle version.
5. **Téléchargez** n'importe quelle version depuis l'historique.
6. **Formulaires** : menu « Formulaires » → construisez vos champs, enregistrez, puis
   partagez le **lien réduit**. Les réponses sont consultables dans la fiche du formulaire.
